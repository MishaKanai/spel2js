...
describe("spel expression evaluator", () => {
    ...
    describe("parse", () => {
        ...
        describe("lookups", () => {
            ...
            it("should look up a doubly nested primitive in the context using dot notation", () => {
                //when
                let bool = evaluator.eval(
                    "nested.reallyNested.iAmTrue",
                    context
                );
            ...
            it("should look up a nested primitive in the context using bracket notation", () => {
                //when
                let string = evaluator.eval(
                    "nested[iAmANestedPropertyName]",
                    context
                );
            ...
            it("should look up a really nested primitive in the context using bracket notation", () => {
                //when
                let string = evaluator.eval(
                    "nested.reallyNested[nested.iAmAString]",
                    context
                );
        ...
        describe("method invocation", () => {
            ...
            it("should look up and invoke a function with arguments", () => {
                //when
                let ret = evaluator.eval(
                    'argumentative("i disagree!")',
                    context
                );
        ...
        describe("matches", () => {
            it("should return true if the left side matches the regexp string on the right side", () => {
                //when
                let matches = evaluator.eval(
                    '"the quick brown fox" matches "^the.*fox$"'
                );
            ...
            it("should return false if the left side does not match the regexp string on the right side", () => {
                //when
                let matches = evaluator.eval(
                    '"the quick brown dog" matches "^the.*fox$"'
                );
        ...
        describe("ternary", () => {
            ...
            it("should return expression if truthy, or ifFalseExpression if null ", () => {
                //when
                let elvisTruthy = evaluator.eval(
                    '"Thank you." ?: "Thank you very much."'
                );
                let elvisFalsy = evaluator.eval(
        ...
        describe("selection/projection", () => {
            it("should return a new list based on selection expression", () => {
                ...
                //when
                let newCollection = evaluator.eval(
                    "collection.?[#this <= 3]",
                    context
                );
            ...
            it("should return a new map based on selection expression", () => {
                ...
                //when
                let newCollection1 = evaluator.eval(
                    "collection.?[value <= 3]",
                    context
                );
                let newCollection2 = evaluator.eval(
                    'collection.?[key == "a"]',
                    context
                );
    ...
    describe("lookups", () => {
        ...
        beforeEach(() => {
            context = {
                iAmANull: null,
                iAmANestedPropertyName: "iAmANull",
                nested: {
                    iAmANull: null
                }
            };
        ...
        it("variable evaluates null variable in the context without throwing NullPointerException", () => {
            //when
            let number = evaluator.eval("iAmANull", context);

            //then
            expect(number).toBe(null);
        });

        it("should look up a nested null value in the context using dot notation", () => {
            //when
            let string = evaluator.eval("nested.iAmANull", context);

            //then
            expect(string).toBe(null);
        });
        it("should look up a nested null value in the context using variable index", () => {
            //when
            let string = evaluator.eval(
                "nested[iAmANestedPropertyName]",
                context
