...
var NumberLiteral_1 = require("./ast/NumberLiteral");
var StringLiteral_1 = require("./ast/StringLiteral");
var NullLiteral_1 = require("./ast/NullLiteral");
var FunctionReference_1 = require("./ast/FunctionReference");
var MethodReference_1 = require("./ast/MethodReference");
var PropertyReference_1 = require("./ast/PropertyReference");
var VariableReference_1 = require("./ast/VariableReference");
var CompoundExpression_1 = require("./ast/CompoundExpression");
var Indexer_1 = require("./ast/Indexer");
var Assign_1 = require("./ast/Assign");
var OpEQ_1 = require("./ast/OpEQ");
var OpNE_1 = require("./ast/OpNE");
var OpGE_1 = require("./ast/OpGE");
var OpGT_1 = require("./ast/OpGT");
var OpLE_1 = require("./ast/OpLE");
var OpLT_1 = require("./ast/OpLT");
var OpPlus_1 = require("./ast/OpPlus");
var OpMinus_1 = require("./ast/OpMinus");
var OpMultiply_1 = require("./ast/OpMultiply");
var OpDivide_1 = require("./ast/OpDivide");
var OpModulus_1 = require("./ast/OpModulus");
var OpPower_1 = require("./ast/OpPower");
var OpInc_1 = require("./ast/OpInc");
var OpDec_1 = require("./ast/OpDec");
var OpNot_1 = require("./ast/OpNot");
var OpAnd_1 = require("./ast/OpAnd");
var OpOr_1 = require("./ast/OpOr");
var OpMatches_1 = require("./ast/OpMatches");
var Ternary_1 = require("./ast/Ternary");
var Elvis_1 = require("./ast/Elvis");
var InlineList_1 = require("./ast/InlineList");
var InlineMap_1 = require("./ast/InlineMap");
var Selection_1 = require("./ast/Selection");
var Projection_1 = require("./ast/Projection");
//not yet implemented
var OpInstanceof_1 = require("./ast/OpInstanceof");
var OpBetween_1 = require("./ast/OpBetween");
var TypeReference_1 = require("./ast/TypeReference");
var BeanReference_1 = require("./ast/BeanReference");
var Identifier_1 = require("./ast/Identifier");
var QualifiedIdentifier_1 = require("./ast/QualifiedIdentifier");
var ConstructorReference_1 = require("./ast/ConstructorReference");
var TokenKind = TokenKind_1.TokenKind;
exports.SpelExpressionParser = function() {
  ...
  function parse(expression, context) {
    try {
      ...
      tokenStream = Tokenizer_1.Tokenizer.tokenize(expression);
  ...
  function eatExpression() {
    ...
    if (moreTokens()) {
      ...
      if (token.getKind() === TokenKind.ASSIGN) {
        // a=b
        if (expr === null) {
          expr = NullLiteral_1.NullLiteral.create(
            toPosBounds(token.startPos - 1, token.endPos - 1)
          );
        }
        nextToken();
        var assignedValue = eatLogicalOrExpression();
        return Assign_1.Assign.create(toPosToken(token), expr, assignedValue);
      }
      if (token.getKind() === TokenKind.ELVIS) {
        // a?:b (a if it isn't null, otherwise b)
        if (expr === null) {
          expr = NullLiteral_1.NullLiteral.create(
            toPosBounds(token.startPos - 1, token.endPos - 2)
          );
        }
        nextToken(); // elvis has left the building
        var valueIfNull = eatExpression();
        if (valueIfNull === null) {
          valueIfNull = NullLiteral_1.NullLiteral.create(
            toPosBounds(token.startPos + 1, token.endPos + 1)
          );
        }
        return Elvis_1.Elvis.create(toPosToken(token), expr, valueIfNull);
      }
      if (token.getKind() === TokenKind.QMARK) {
        ...
        return Ternary_1.Ternary.create(
          toPosToken(token),
          expr,
          ifTrueExprValue,
          ifFalseExprValue
  ...
  function eatLogicalOrExpression() {
    ...
    while (peekIdentifierToken("or") || peekTokenOne(TokenKind.SYMBOLIC_OR)) {
      ...
      expr = OpOr_1.OpOr.create(toPosToken(token), expr, rhExpr);
    }
    return expr;
  }
  ...
  function eatLogicalAndExpression() {
    ...
    while (peekIdentifierToken("and") || peekTokenOne(TokenKind.SYMBOLIC_AND)) {
      ...
      checkOperands(token, expr, rhExpr);
      expr = OpAnd_1.OpAnd.create(toPosToken(token), expr, rhExpr);
    }
    return expr;
  }
  ...
  function eatRelationalExpression() {
    ...
    if (relationalOperatorToken !== null) {
      ...
      if (relationalOperatorToken.isNumericRelationalOperator()) {
        ...
        if (tk === TokenKind.GT) {
          return OpGT_1.OpGT.create(pos, expr, rhExpr);
        }
        if (tk === TokenKind.LT) {
          return OpLT_1.OpLT.create(pos, expr, rhExpr);
        }
        if (tk === TokenKind.LE) {
          return OpLE_1.OpLE.create(pos, expr, rhExpr);
        }
        if (tk === TokenKind.GE) {
          return OpGE_1.OpGE.create(pos, expr, rhExpr);
        }
        if (tk === TokenKind.EQ) {
          return OpEQ_1.OpEQ.create(pos, expr, rhExpr);
        }
        //Assert.isTrue(tk === TokenKind.NE);
        return OpNE_1.OpNE.create(pos, expr, rhExpr);
      }
      if (tk === TokenKind.INSTANCEOF) {
        return OpInstanceof_1.OpInstanceof.create(
          toPosToken(token),
          expr,
          rhExpr
        );
      ...
      if (tk === TokenKind.MATCHES) {
        return OpMatches_1.OpMatches.create(toPosToken(token), expr, rhExpr);
      }
      //Assert.isTrue(tk === TokenKind.BETWEEN);
      return OpBetween_1.OpBetween.create(toPosToken(token), expr, rhExpr);
    }
    return expr;
  }
  ...
  function eatSumExpression() {
    ...
    while (peekTokenAny(TokenKind.PLUS, TokenKind.MINUS, TokenKind.INC)) {
      ...
      if (token.getKind() === TokenKind.PLUS) {
        expr = OpPlus_1.OpPlus.create(toPosToken(token), expr, rhExpr);
      } else if (token.getKind() === TokenKind.MINUS) {
        expr = OpMinus_1.OpMinus.create(toPosToken(token), expr, rhExpr);
      }
    }
    return expr;
  }
  ...
  function eatProductExpression() {
    ...
    while (peekTokenAny(TokenKind.STAR, TokenKind.DIV, TokenKind.MOD)) {
      ...
      if (token.getKind() === TokenKind.STAR) {
        expr = OpMultiply_1.OpMultiply.create(toPosToken(token), expr, rhExpr);
      } else if (token.getKind() === TokenKind.DIV) {
        expr = OpDivide_1.OpDivide.create(toPosToken(token), expr, rhExpr);
      } else {
        //Assert.isTrue(token.getKind() === TokenKind.MOD);
        expr = OpModulus_1.OpModulus.create(toPosToken(token), expr, rhExpr);
      }
    }
    return expr;
  }
  ...
  function eatPowerIncDecExpression() {
    ...
    if (peekTokenOne(TokenKind.POWER)) {
      ...
      checkRightOperand(token, rhExpr);
      return OpPower_1.OpPower.create(toPosToken(token), expr, rhExpr);
    }
    if (expr !== null && peekTokenAny(TokenKind.INC, TokenKind.DEC)) {
      token = nextToken(); //consume INC/DEC
      if (token.getKind() === TokenKind.INC) {
        return OpInc_1.OpInc.create(toPosToken(token), true, expr);
      }
      return OpDec_1.OpDec.create(toPosToken(token), true, expr);
    }
    return expr;
  }
  ...
  function eatUnaryExpression() {
    ...
    if (peekTokenAny(TokenKind.PLUS, TokenKind.MINUS, TokenKind.NOT)) {
      ...
      if (token.getKind() === TokenKind.NOT) {
        return OpNot_1.OpNot.create(toPosToken(token), expr);
      }
      if (token.getKind() === TokenKind.PLUS) {
        return OpPlus_1.OpPlus.create(toPosToken(token), expr);
      }
      //Assert.isTrue(token.getKind() === TokenKind.MINUS);
      return OpMinus_1.OpMinus.create(toPosToken(token), expr);
    }
    if (peekTokenAny(TokenKind.INC, TokenKind.DEC)) {
      token = nextToken();
      expr = eatUnaryExpression();
      if (token.getKind() === TokenKind.INC) {
        return OpInc_1.OpInc.create(toPosToken(token), false, expr);
      }
      return OpDec_1.OpDec.create(toPosToken(token), false, expr);
    }
  ...
  function eatPrimaryExpression() {
    ...
    return CompoundExpression_1.CompoundExpression.create(
      toPosBounds(
        start.getStartPosition(),
        nodes[nodes.length - 1].getEndPosition()
      ),
      nodes
    );
  }
  ...
  function eatDottedNode() {
    ...
    if (
      maybeEatMethodOrProperty(nullSafeNavigation) ||
  ...
  function maybeEatFunctionOrVar() {
    ...
    if (args === null) {
      push(
        VariableReference_1.VariableReference.create(
          functionOrVariableName.data,
          toPosBounds(token.startPos, functionOrVariableName.endPos)
        )
      );
      return true;
    }
    push(
      FunctionReference_1.FunctionReference.create(
        functionOrVariableName.data,
        toPosBounds(token.startPos, functionOrVariableName.endPos),
        args
      )
  ...
  function eatStartNode() {
    if (maybeEatLiteral()) {
      ...
      maybeEatProjection(false) ||
      maybeEatSelection(false) ||
      maybeEatIndexer()
    ) {
      return pop();
  ...
  function maybeEatBeanReference() {
    if (peekTokenOne(TokenKind.BEAN_REF)) {
      ...
      var beanReference = BeanReference_1.BeanReference.create(
        toPosToken(beanNameToken),
        beanName
      );
      push(beanReference);
      return true;
  ...
  function maybeEatTypeReference() {
    if (peekTokenOne(TokenKind.IDENTIFIER)) {
      ...
      if (peekTokenOne(TokenKind.RSQUARE)) {
        ...
        push(
          PropertyReference_1.PropertyReference.create(
            token.stringValue(),
            toPosToken(token)
          )
        );
      ...
      eatToken(TokenKind.RPAREN);
      push(
        TypeReference_1.TypeReference.create(toPosToken(typeName), node, dims)
      );
      return true;
    }
    return false;
  ...
  function maybeEatNullReference() {
    if (peekTokenOne(TokenKind.IDENTIFIER)) {
      ...
      nextToken();
      push(NullLiteral_1.NullLiteral.create(toPosToken(nullToken)));
      return true;
    }
    return false;
  }
  ...
  function maybeEatProjection(nullSafeNavigation) {
    ...
    eatToken(TokenKind.RSQUARE);
    push(
      Projection_1.Projection.create(
        nullSafeNavigation,
        toPosToken(token),
        expr
      )
  ...
  function maybeEatInlineListOrMap() {
    ...
    if (peekTokenConsumeIfMatched(TokenKind.RCURLY, true)) {
      // empty list '{}'
      expr = InlineList_1.InlineList.create(
        toPosBounds(token.startPos, closingCurly.endPos)
      );
    } else if (peekTokenConsumeIfMatched(TokenKind.COLON, true)) {
      closingCurly = eatToken(TokenKind.RCURLY);
      // empty map '{:}'
      expr = InlineMap_1.InlineMap.create(
        toPosBounds(token.startPos, closingCurly.endPos)
      );
    } else {
      ...
      if (peekTokenOne(TokenKind.RCURLY)) {
        ...
        closingCurly = eatToken(TokenKind.RCURLY);
        expr = InlineList_1.InlineList.create(
          toPosToken(token.startPos, closingCurly.endPos),
          listElements
        );
      } else if (peekTokenConsumeIfMatched(TokenKind.COLON, true)) {
        // map!
  ...
  function maybeEatIndexer() {
    ...
    var expr = eatExpression();
    eatToken(TokenKind.RSQUARE);
    push(Indexer_1.Indexer.create(toPosToken(token), expr));
    return true;
  }
  function maybeEatSelection(nullSafeNavigation) {
    ...
    if (token.getKind() === TokenKind.SELECT_FIRST) {
      push(
        Selection_1.Selection.create(
          nullSafeNavigation,
          Selection_1.Selection.FIRST,
          toPosToken(token),
          expr
        )
      );
  ...
  function eatPossiblyQualifiedId() {
    ...
    while (isValidQualifiedId(node)) {
      ...
      if (node.kind !== TokenKind.DOT) {
        qualifiedIdPieces.push(
          Identifier_1.Identifier.create(node.stringValue(), toPosToken(node))
        );
      }
      node = peekToken();
    }
    ...
    var pos = toPosBounds(
      qualifiedIdPieces[0].getStartPosition(),
      qualifiedIdPieces[qualifiedIdPieces.length - 1].getEndPosition()
    );
    return QualifiedIdentifier_1.QualifiedIdentifier.create(
      pos,
      qualifiedIdPieces
    );
  }
  ...
  function maybeEatMethodOrProperty(nullSafeNavigation) {
    if (peekTokenOne(TokenKind.IDENTIFIER)) {
      ...
      // methodreference
      push(
        MethodReference_1.MethodReference.create(
          nullSafeNavigation,
          methodOrPropertyName.stringValue(),
          toPosToken(methodOrPropertyName),
          args
  ...
  function maybeEatConstructorReference() {
    if (peekIdentifierToken("new")) {
      ...
      if (peekTokenOne(TokenKind.LSQUARE)) {
        ...
        push(
          ConstructorReference_1.ConstructorReference.create(
            toPosToken(newToken),
            dimensions,
            nodes
          )
  ...
  function maybeEatLiteral() {
    ...
    if (
      token.getKind() === TokenKind.LITERAL_INT ||
      token.getKind() === TokenKind.LITERAL_LONG
    ) {
      push(
        NumberLiteral_1.NumberLiteral.create(
          parseInt(token.stringValue(), 10),
          toPosToken(token)
        )
      );
    } else if (
      token.getKind() === TokenKind.LITERAL_REAL ||
      token.getKind() === TokenKind.LITERAL_REAL_FLOAT
    ) {
      push(
        NumberLiteral_1.NumberLiteral.create(
          parseFloat(token.stringValue()),
          toPosToken(token)
        )
      );
    } else if (
      token.getKind() === TokenKind.LITERAL_HEXINT ||
      token.getKind() === TokenKind.LITERAL_HEXLONG
    ) {
      push(
        NumberLiteral_1.NumberLiteral.create(
          parseInt(token.stringValue(), 16),
          toPosToken(token)
        )
      );
    } else if (peekIdentifierToken("true")) {
      push(BooleanLiteral_1.BooleanLiteral.create(true, toPosToken(token)));
    } else if (peekIdentifierToken("false")) {
      push(BooleanLiteral_1.BooleanLiteral.create(false, toPosToken(token)));
    } else if (token.getKind() === TokenKind.LITERAL_STRING) {
      push(
        StringLiteral_1.StringLiteral.create(
          token.stringValue(),
          toPosToken(token)
        )
      );
  ...
  function peekTokenConsumeIfMatched(desiredTokenKind, consumeIfMatched) {
    ...
    if (desiredTokenKind === TokenKind.IDENTIFIER) {
      ...
      if (
        token.getKind().ordinal() >= TokenKind.DIV.ordinal() &&
        token.getKind().ordinal() <= TokenKind.NOT.ordinal() &&
        token.data !== null
      ) {
  ...
  function peekIdentifierToken(identifierString) {
    ...
    return (
      ...
      token.stringValue().toLowerCase() === identifierString.toLowerCase()
    );
  }
  function peekSelectToken() {
    ...
    var token = peekToken();
    return (
      token.getKind() === TokenKind.SELECT ||
      token.getKind() === TokenKind.SELECT_FIRST ||
  ...
  function raiseInternalException(pos, message, expected, actual) {
    ...
    throw {
      ...
      message:
        "Error occurred while attempting to parse expression '" +
        expressionString +
        "' at position " +
        pos +
        ". Message: " +
...
//# sourceMappingURL=SpelExpressionParser.js.map
