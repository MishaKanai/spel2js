...
function createNode(position, expressionComponents) {
  var node = SpelNode.create.apply(
    null,
    ["compound", position].concat(expressionComponents)
  );
  ...
  function buildContextStack(state) {
    ...
    for (i = 0; i < childrenCount; i += 1) {
      if (node.getChildren()[i].getType() === "indexer") {
        state.activeContext.push(
          state.activeContext.peek()[node.getChildren()[i].getValue(state)]
        );
      } else {
  ...
  node.getValue = function(state) {
    ...
    if (!context) {
      throw {
        ...
        message:
          "Attempting to evaluate compound expression with an undefined context."
      };
    }
