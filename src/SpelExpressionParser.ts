...
import { TokenKind, Tokens, ITokenKind } from "./TokenKind";
...
export var SpelExpressionParser = function() {
  ...
  function eatExpression() {
    ...
    if (moreTokens()) {
      ...
      if (token.getKind() === TokenKind.ELVIS) {
        ...
        if (valueIfNull === null) {
          valueIfNull = NullLiteral.create(
            toPosBounds(token.startPos + 1, token.endPos + 1)
          );
        }
  ...
  function maybeEatBeanReference() {
    if (peekTokenOne(TokenKind.BEAN_REF)) {
      ...
      var beanReference = BeanReference.create(
  ...
  function maybeEatInlineListOrMap() {
    ...
    if (peekTokenConsumeIfMatched(TokenKind.RCURLY, true)) {
      ...
      if (peekTokenOne(TokenKind.RCURLY)) {
        ...
        closingCurly = eatToken(TokenKind.RCURLY);
        expr = InlineList.create(
          // maybe it was meant to be ToPosBounds????? INVESTIGATE
          toPosBounds(token.startPos, closingCurly.endPos), // BUG FOUND, was  , toPosToken(token.startPos, closingCurly.endPos),
          listElements
        );
      } else if (peekTokenConsumeIfMatched(TokenKind.COLON, true)) {
        // map!
    ...
    while (isValidQualifiedId(node)) {
      ...
      if (node.kind !== TokenKind.DOT) {
        qualifiedIdPieces.push(
          Identifier.create(node.stringValue(), toPosToken(node))
        );
      }
      node = peekToken();
    }
  ...
  function maybeEatConstructorReference() {
    if (peekIdentifierToken("new")) {
      ...
      if (peekTokenOne(TokenKind.LSQUARE)) {
        ...
        push(
          ConstructorReference.create(toPosToken(newToken), dimensions, nodes)
        );
      } else {
  ...
  function maybeEatLiteral() {
    ...
    if (
      ...
      push(
        NumberLiteral.create(parseFloat(token.stringValue()), toPosToken(token))
      );
    } else if (
  ...
  function peekTokenAny(...args: ITokenKind[]) {
    if (!moreTokens()) {
      return false;
    }
    var token = peekToken();
  ...
  function raiseInternalException(
    pos,
    message,
    expected?: string,
    actual?: string
